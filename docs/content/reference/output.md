...
describes what users get. Pick a format with `-o`, or let juejin choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
juejin <command> -o table   # aligned columns for reading
juejin <command> -o jsonl   # one JSON object per line, for piping
juejin <command> -o json    # a single JSON array
juejin <command> -o csv     # spreadsheet friendly
juejin <command> -o tsv     # tab-separated
juejin <command> -o url     # just the URL column
juejin <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
juejin <command> --fields id,title,url
```
...
```bash
juejin <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
juejin <command>            # a table, because this is a terminal
juejin <command> | wc -l    # JSONL, because this is a pipe
```
