// Package cli builds the juejin command tree on top of the juejin library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: juejin.DefaultConfig()}

	root := &cobra.Command{
		Use:   "juejin",
		Short: "Browse Juejin developer articles (掘金)",
		Long: `juejin reads Juejin (掘金) through its public recommendation API.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

juejin is an independent tool and is not affiliated with Juejin.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = juejin.NewClient(a.cfg)
	return nil
}
